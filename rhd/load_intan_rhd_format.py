...
def read_data(filename, no_floats=False):
    ...
    if (data_present):
        ...
        if no_floats:
            ...
            data['amplifier_bit_microvolts'] = AMPLIFIER_BIT_MICROVOLTS
            data['amplifier_data'] = (data['amplifier_data'].astype(np.int32)
                                      - UINT16_BIT_OFFSET).astype(np.int16)
            data['aux_bit_volts'] = AUX_BIT_VOLTS
            data['supply_bit_volts'] = SUPPLY_BIT_VOLTS
            data['temp_bit_celcius'] = TEMP_BIT_CELCIUS

            if header['eval_board_mode'] == 1:
                data['ADC_input_bit_volts'] = ADC_BIT_VOLTS_1
                data['board_adc_data'] = (data['board_adc_data'].astype(np.int32) - UINT16_BIT_OFFSET).astype(np.int16)
            else:
                data['ADC_input_bit_volts'] = ADC_BIT_VOLTS_0
        else:
            # Scale voltage levels appropriately.
            data['amplifier_data'] = np.multiply(AMPLIFIER_BIT_MICROVOLTS, (
                data['amplifier_data'].astype(np.int32) - UINT16_BIT_OFFSET)
                                                )  # units = microvolts
            data['aux_input_data'] = np.multiply(AUX_BIT_VOLTS, data['aux_input_data']
                                                )  # units = volts
            data['supply_voltage_data'] = np.multiply(
                SUPPLY_BIT_VOLTS, data['supply_voltage_data'])  # units = volts
            if header['eval_board_mode'] == 1:
                data['board_adc_data'] = np.multiply(ADC_BIT_VOLTS_1, (
                    data['board_adc_data'].astype(np.int32) - UINT16_BIT_OFFSET)
                                                    )  # units = volts
            else:
                data['board_adc_data'] = np.multiply(
                    ADC_BIT_VOLTS_0, data['board_adc_data'])  # units = volts
            data['temp_sensor_data'] = np.multiply(TEMP_BIT_CELCIUS, data['temp_sensor_data']
                                                )  # units = deg C
